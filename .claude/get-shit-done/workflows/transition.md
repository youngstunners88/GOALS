...
```bash
TRANSITION=$(node "/home/teacherchris37/soccer-souls/.claude/get-shit-done/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "/home/teacherchris37/soccer-souls/.claude/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
if [ -n "$GSD_WORKSTREAM" ]; then
  WS_LIST=$(node "/home/teacherchris37/soccer-souls/.claude/get-shit-done/bin/gsd-tools.cjs" workstream list --raw)
fi
```
...
```bash
node "/home/teacherchris37/soccer-souls/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
